...
export class CarritoPage implements OnInit {
  ...
  constructor(private _servicio_tienda: TiendaService, private navCtrl: NavController,
    ...
    //console.log("iniciando carrito page DESDE CONSTRUCTOR");
  }

  ngOnInit() {
    //console.log("iniciando carrito page DESDE ONINIT");
  ...
  ionViewWillEnter(){
    console.log("se ejecuta cada vez que ingresa a la pagina");
   this.getCalculos(this._servicio_carrito.carrito, this.cupon);
  }
  ...
  loadListPagos() {
    this._servcio_producto.getListTipoPagos().subscribe(
      res => {
        ...
        console.log("loadListPagos(): " + JSON.stringify(res));
      }
    );
  }
  ...
  openPedido() {
    this._servicio_tienda.getTraerCoordenadasTienda().subscribe(
      res => {
        console.log("corrdeandas tienda = " + JSON.stringify(res))
        this.abrirModalPedido(res.latitud, res.longitud);
      },
  ...
  async abrirModalPedido(latitud: any, longitud: any) {
    const myModal = await this.viewCtrl.create({
      ...
      componentProps: {
        latitudtienda: latitud, longitudtienda: longitud, tipopago: this.tipoPago.nombre,
